...
export const Receivables: React.FC<ReceivablesProps> = ({ receivables, state, onAdd, onUpdate, onDelete }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const defaultAccountId = state.accounts.length > 0 ? state.accounts[0].id : undefined;
    ...
    <div className="space-y-6">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingItem ? "Edit Receipt" : "Record Receipt"}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Received From (Customer)</label>
          ...
          <div>
            ...
            <select required className="w-full p-2 border border-gray-300 rounded-lg" value={formData.accountId} onChange={e => setFormData({...formData, accountId: e.target.value})}>
          ...
          <div>
            ...
            <input required type="number" step="0.01" placeholder="0.00" className="w-full p-2 border border-gray-300 rounded-lg" value={formData.amount} onChange={e => setFormData({...formData, amount: e.target.value})} />
          </div>
